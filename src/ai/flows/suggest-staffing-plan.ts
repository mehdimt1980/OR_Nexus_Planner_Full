...
// Backward compatibility function for existing useORData hook
export async function suggestStaffingPlanLegacy(legacyInput: {
  ...
  const operations = legacyInput.operatingRooms.map((room, index) => ({
    id: `${room.name}-${room.shift}-${index}`,
    room: room.name,
    department: 'ACH', // Default department for legacy calls
    scheduledTime: room.shift === 'BD1' ? '07:30' : room.shift === 'BD2' ? '12:00' : '16:00',
    procedureName: 'Allgemeine Operation',
    operationComplexity: room.operationComplexity,
    estimatedDuration: 90,
  }));

  const availableStaff = legacyInput.availableStaff.map(name => ({
    name,
    departmentExpertise: ['Allgemein'],
    skills: [],
    currentAssignments: [],
  }));

  const newInput: SuggestStaffingPlanInput = {
    operations,
    availableStaff,
    sickStaff: legacyInput.sickStaff,
  };

  const result = await suggestStaffingPlanFlow(newInput);
  
  // Convert back to legacy format
  return {
    assignments: result.assignments.map(assignment => ({
      operatingRoom: assignment.room,
      shift: operations.find(op => op.id === assignment.operationId)?.scheduledTime.includes('07:') ? 'BD1' : 
             operations.find(op => op.id === assignment.operationId)?.scheduledTime.includes('12:') ? 'BD2' : 'BD3',
      staff: assignment.assignedStaff.map(staff => staff.name),
